...
def preprocess_screening_data(df2):
    ...
    df2 = df2.drop(['Timestamp', 'Full Name:', 'Username', 'Prediction', 'Confidence'], axis=1)

    # Step 2: Get the column names (questions)
    questions = pd.Series(df2.columns)

    # Step 3: Define question types
    quest_type = {
    ...
    # Step 5: Define function to get the original question from the new name
    def get_question(question: str):
        idx = question.split('_')[-1]
        return questions[eval(idx)]

    # Step 6: Identify columns to drop based on "open" and not equal to "application_open_31"
    columns_to_drop = [col for col in df2.columns if "open" in col and col != "application_open_31"]

    # Add specific "application_close" columns to drop
    columns_to_drop += ["application_close_33", "application_close_35", "application_close_37", "application_close_39", "application_close_41"]

    # Step 7: Drop the identified columns
    df2 = df2.drop(columns=columns_to_drop)

    # Step 8: Standardize variations of 'None' across the data using a vectorized approach
    none_variations = ['none', 'NONE', 'n/a', 'N/A', '']
    df2 = df2.replace(none_variations + [np.nan], 'None')

    # Step 9: Handle previous pet-related columns where 'Previous Pet Species' is 'None'
    ...
    # Step 10: Fill 'None' for specific columns with null values
    fill_none = ['application_close_27', 'application_close_29']
    df2[fill_none] = df2[fill_none].fillna('None')

    # Step 11: Fill 'No Response' for another set of columns with null values
...
def preprocess_recommendation_data(df3):
    df3 = df3.drop(['Timestamp', 'Email address'], axis=1)
...
def assign_adopter_category(previous_pet):
    ...
    elif 'application_close_19_Dog' in previous_pet.columns:
        previous_pet['adopter_category'] = previous_pet['application_close_19_Dog']
    elif 'application_close_19_Cat' in previous_pet.columns:
        previous_pet['adopter_category'] = previous_pet['application_close_19_Cat']
    else:
        raise ValueError("'adopter_category' column is missing in previous_pet DataFrame.")
...
def run_pipeline(adopter_id, pet_data, user_profile, previous_pet, desired_pet, df2, df3):
    """
    ...
    user_profile['adopter_id'] = adopter_id
    previous_pet['adopter_id'] = adopter_id
    desired_pet['adopter_id'] = adopter_id
...
def recommend_pets_with_features(adopter_id, top_n=5, threshold=0.4, recommended_pet_ids=None, user_profile=None, df1=None, df2=None, df3=None, previous_pet=None, desired_pet=None, pet_data=None):
    ...
    if adopter_category == 1.0:
        experience_recommended = _recommend_for_experienced(adopter_row, top_n, threshold, recommended_pet_ids, adopter_preferences, previous_pet, desired_pet, pet_data, df1, user_profile, type_mapping)
        print(experience_recommended)
        return experience_recommended
    
    if adopter_category == '0':
        new_recommended = _recommend_for_new(adopter_row, top_n, threshold, recommended_pet_ids, adopter_preferences, previous_pet, desired_pet, pet_data, df1, user_profile, type_mapping)
...
def _recommend_for_new(adopter_row, top_n, threshold, recommended_pet_ids, adopter_preferences, previous_pet, desired_pet, pet_data, df1, user_profile, type_mapping):
    ...
    item_based_similarity = compute_similarity(
        desired_pet[desired_pet['adopter_category'] == 1].iloc[:, 1:], pet_data.iloc[:, 1:]
    )
...
def get_top_n_pets_with_fallback(similarity_scores, top_n, threshold, recommended_pet_ids, df1, user_profile, adopter_preferences, type_mapping):
    ...
    # Ensure desired_pet_type is valid
    if desired_pet_type not in ['Dog', 'Cat']:
        raise ValueError(f"Invalid desired pet type: {desired_pet_type}")

    # Correct type mapping and apply it to the 'Type' column in df1
    type_mapping = {'Cat': 0, 'Dog': 1}  # Map pet type strings to numeric codes
    df1['Type'] = df1['Type'].map({v: k for k, v in type_mapping.items()})  # Reverse mapping

    # Filter pets based on the adopter's desired pet type
    filtered_pets = df1[df1['Type'] == desired_pet_type]
    ...
    for pet_idx in sorted_indices[:top_n]:
        ...
        if similarity_scores[pet_idx] >= threshold and pet_id not in recommended_pet_ids:
            ...
            print(f"Type: {type_mapping.get(filtered_pets.loc[pet_id, 'Type'], 'Unknown')}")
...
def process_and_recommend(
    ...
    # Load data
    df1, df2, df3 = load_data(pet_file, screening_file, recommendation_file)
    
    # Preprocess data
    df2 = preprocess_screening_data(df2)
    df3 = preprocess_recommendation_data(df3)
    
    # Process data
    pet_data, user_profile, previous_pet, desired_pet = run_process(df1, df2, df3)
    
    # Run pipeline
    pet_data, user_profile, previous_pet, desired_pet, df2, df3 = run_pipeline(
        adopter_id, pet_data, user_profile, previous_pet, desired_pet, df2, df3
    )
    
    # Generate recommendations
    recommended_pets = recommend_pets_with_features(
...
if __name__ == "__main__":
    ...
    try:
        recommendations = process_and_recommend(pet_file, screening_file, recommendation_file, adopter_id)
        # Convert numpy integers to Python integers for JSON serialization
        recommendations = [int(pet_id) for pet_id in recommendations]
