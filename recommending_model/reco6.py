...
def preprocess_screening_data(df2):
    ...
    for col in columns_with_extra_commas:
...
# full  function
def process_and_recommend(pet_file, screening_file, recommendation_file, adopter_id, top_n=5, threshold=0.4):
    try:
        # Load data
        pet_df, screening_df, recommendation_df = load_data(pet_file, screening_file, recommendation_file)
        
        # Preprocess data
        screening_df = preprocess_screening_data(screening_df)
        recommendation_df = preprocess_recommendation_data(recommendation_df)
        
        # Extract adopter-specific data
        adopter_data = screening_df[screening_df['adopter_id'] == adopter_id]
        if adopter_data.empty:
            raise ValueError(f"No screening data found for adopter_id {adopter_id}")
        
        desired_pet_data = recommendation_df[recommendation_df['adopter_id'] == adopter_id]
        if desired_pet_data.empty:
            raise ValueError(f"No recommendation data found for adopter_id {adopter_id}")
        
        # Compute similarity and recommend pets
        similarity_scores = cosine_similarity(pet_df.iloc[:, 1:], desired_pet_data.iloc[:, 1:])
        recommended_pets = []
        
        for idx, score in enumerate(similarity_scores.flatten()):
            if score >= threshold:
                pet_id = pet_df.index[idx]
                recommended_pets.append({
                    "pet_id": int(pet_id),
                    "score": float(score),
                    "details": pet_df.iloc[idx].to_dict()
                })
            if len(recommended_pets) >= top_n:
                break

        # Fallback if no pets meet the threshold
        if not recommended_pets:
            ...
            for idx in sorted_indices[:top_n]:
                ...
                recommended_pets.append({
                    "pet_id": int(pet_id),
                    "score": float(similarity_scores.flatten()[idx]),
                    "details": pet_df.iloc[idx].to_dict()
                })
        ...
        return json.dumps({
            "status": "success",
            "recommendations": recommended_pets
        })
