...
def preprocess_screening_data(df2):
    ...
    # Step 12: Filter the DataFrame to keep only those with 'Approve' in 'application_close_43'
    df2 = df2[df2['application_close_43'] == 'Approve'].reset_index(drop=True)

    # Step 13: Drop the 'application_close_43' column
    df2 = df2.drop(columns=['application_close_43'])

    return df2

# Preprocessing for Recommendation Model (df3)
def preprocess_recommendation_data(df3):
    if 'Email address' in df3.columns:
        email_column = df3['Email address']
    else:
        email_column = None

    # Original preprocessing
    df3.drop(columns=['Timestamp'], inplace=True, errors='ignore')

    # Add 'Email address' back if needed
    if email_column is not None:
        df3['Email address'] = email_column
...
def process_and_recommend(
    ...
    try:
        ...
        recommendations = recommend_pets_with_features(
            adopter_id, top_n, threshold,
            user_profile=None ,df1=pet_df, df2=screening_df, df3=recommendation_df
        )

        # Save recommendations to a temporary file
        with tempfile.NamedTemporaryFile(delete=False, suffix=".csv") as temp_file:
            recommendation_df.to_csv(temp_file.name, index=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = process_and_recommend(pet_file, screening_file, recommendation_file, adopter_id)
        
        # Print the output for the caller (Node.js controller)
        print(result)
