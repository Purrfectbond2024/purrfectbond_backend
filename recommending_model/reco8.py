...
def load_data(pet_file, screening_file, recommendation_file):
    ...
    return pet_df, screening_df, recommendation_df  

# Example preprocessing function
def preprocess_data(pet_df, screening_df, recommendation_df):
    """
    Preprocess the data loaded from CSV files.
    Args:
        pet_df (DataFrame): Pet data.
        screening_df (DataFrame): Screening data.
        recommendation_df (DataFrame): Recommendation data.
    Returns:
        Tuple[DataFrame, DataFrame, DataFrame]: Preprocessed data.
    """
    # Preprocess pet data
    pet_df['Type'] = pet_df['Type'].str.strip().str.capitalize()
    pet_df['Age'] = pet_df['Age'].str.strip().str.capitalize()
    pet_df['Gender'] = pet_df['Gender'].str.strip().str.capitalize()
    pet_df['Vaccinated'] = pet_df['Vaccinated'].str.strip().str.capitalize()
    pet_df['Spayed/Neutered'] = pet_df['Spayed/Neutered'].str.strip().str.capitalize()
    pet_df['AdoptionStory'] = pet_df['AdoptionStory'].str.strip().str.capitalize()
    pet_df['Extracted_Temperaments'] = pet_df['Extracted_Temperaments'].str.lower()

    # Example of handling missing data
    pet_df.fillna('Unknown', inplace=True)

    # Screening and recommendation preprocessing can be added here as needed
    return pet_df, screening_df, recommendation_df

# Example recommendation function
def recommend_pets(pet_df, screening_df, recommendation_df, adopter_id, top_n, threshold):
    """
    Generate pet recommendations based on adopter preferences and confidence scores.

    Args:
        pet_df (DataFrame): Pet data.
        screening_df (DataFrame): Screening data.
        recommendation_df (DataFrame): Recommendation data.
        adopter_id (int): Adopter ID for filtering preferences.
        top_n (int): Number of top recommendations.
        threshold (float): Confidence threshold for recommendations.

    Returns:
        List[dict]: Recommended pets with their scores.
    """
    # Retrieve adopter preferences from recommendation_df
    adopter_preferences = recommendation_df.iloc[0]  # Assuming one row per adopter

    # Retrieve prediction and confidence from screening_df for the given adopter
    adopter_screening = screening_df.loc[screening_df['adopter_id'] == adopter_id]
    if adopter_screening.empty:
        raise ValueError(f"No screening data found for adopter_id {adopter_id}")
    
    confidence = adopter_screening['Confidence'].iloc[0]
    prediction = adopter_screening['Prediction'].iloc[0]

    # Ensure only approved adopters can proceed
    if prediction.lower() != "approve":
        raise ValueError(f"Adopter {adopter_id} is not approved (status: {prediction})")

    # Filter pets based on adopter preferences
    filtered_pets = pet_df[
        (pet_df['Type'] == adopter_preferences['1. What type of pet are you interested in adopting?']) &
        (pet_df['Age'] == adopter_preferences['2. What age range of pet are you interested in?']) &
        ((pet_df['Gender'] == adopter_preferences['3. Do you have a preference for the pet’s gender?']) |
         (adopter_preferences['3. Do you have a preference for the pet’s gender?'] == 'No preference')) &
        (pet_df['Spayed/Neutered'] == adopter_preferences['5. Would you prefer your pet to be spayed/neutered at the time of adoption?']) &
        (pet_df['Vaccinated'] == adopter_preferences['6. Would you prefer your pet to be vaccinated at the time of adoption?'])
    ]

    # Scoring: Incorporate confidence
    filtered_pets['Score'] = confidence  # Base score is the confidence value

    # Filter by threshold and sort by score
    filtered_pets = filtered_pets[filtered_pets['Score'] >= threshold]
    filtered_pets = filtered_pets.sort_values(by='Score', ascending=False)

    # Limit to top_n results
    top_pets = filtered_pets.head(top_n)

    # Convert to dictionary format
    recommendations = top_pets[['Type', 'Age', 'Gender', 'Score']].to_dict(orient='records')

    return recommendations

# Preprocessing for Screening Model (df2)
def preprocess_screening_data(df2):
    if 'adopter_id' in df2.columns:
        adopter_id_col = df2['adopter_id']
    else:
        raise KeyError("'adopter_id' column is missing from the screening data")

    df2 = pd.concat(
        [adopter_id_col, df2.drop(['Full Name:', 'Username', 'Prediction', 'Confidence'], axis=1, errors='ignore')],
        axis=1
    )
    ...
    task_type = {'application': [i for i in range(44)]}
    ...
    # Handle "None" and other missing value replacements
    ...
    for col in previous_pet_columns:
...
def preprocess_recommendation_data(df3):
    ...
    if list(df3.columns) != required_columns:
        raise ValueError(f"Recommendation Data Columns do not match expected format: {required_columns}")
    return df3
...
def run_pipeline(pet_data, user_profile, previous_pet, desired_pet, df2, df3):
    ...
    # Step 5: Add 'adopter_id' to df2 and df3
...
def process_and_recommend(
    ...
    adopter_id: int, top_n: int = 5, threshold: float = 0.4
):
    try:
        # Load and preprocess data
        pet_df, screening_df, recommendation_df = load_data(
            pet_file, screening_file, recommendation_file
        )
        screening_df = preprocess_screening_data(screening_df)
        recommendation_df = preprocess_recommendation_data(recommendation_df)

        # Simulated recommendation logic
        filtered_pets = pet_df.head(top_n).to_dict(orient='records')

        # Save recommendations temporarily
        with tempfile.NamedTemporaryFile(delete=False, suffix=".csv") as temp_file:
            pd.DataFrame(filtered_pets).to_csv(temp_file.name, index=False)
            temp_filename = temp_file.name

        # Return success response
        return json.dumps({
            "status": "success",
            "recommendations": filtered_pets,
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python reco.py <pet_file> <screening_file> <recommendation_file> <adopter_id>")
        sys.exit(1)

    pet_file = sys.argv[1]
    screening_file = sys.argv[2]
    recommendation_file = sys.argv[3]
    adopter_id = int(sys.argv[4])

    try:
        recommendations = process_and_recommend(
            pet_file, screening_file, recommendation_file, adopter_id
        )
